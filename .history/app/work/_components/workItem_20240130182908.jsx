...
function WorkItem() {
  ...
  useEffect(() => {
    ...
    const fetchedWork = data?.filter((work) => {
    ...
    const currentWorkIndex = data.findIndex(
      (item) => item.id === params.workId
    );

    const previousWorkIndex = (currentWorkIndex + 1) % data.length;
    const nextWorkIndex = (currentWorkIndex - 1) % data.length;
    setNextWork(data[nextWorkIndex]);
    setWorkPreviousIndex(data[previousWorkIndex]);
    const image2 = getImageName(workNextWork?.title);
    setImageWork2(image2);

    console.log("next", workNextWork);
    console.log("previous", workPreviousIndex);
  }, [data, params.workId]);
